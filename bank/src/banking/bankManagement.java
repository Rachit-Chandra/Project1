...
public class bankManagement { // these class provides all// bank method
	...
	public static boolean createAccount(String name,int passCode) // create account function
	{
		try {
			...
			if (st.executeUpdate(sql) == 1) {
				System.out.println(name+ ", Registration Successfull Now You can Login!");
				return true;
			}
			// return
		}
	...
	public static boolean loginAccount(String name, int passCode) // login method
	{
		try {
			...
			ResultSet rs = st.executeQuery();// Execution
			BufferedReader sc = new BufferedReader(
				new InputStreamReader(System.in));

			if (rs.next()) {
				// after login menu driven interface method is to be implemented
				...
				while (true) {
					try {
						...
						//deposit
							...
							depo =  Integer.parseInt(sc.readLine());
							
							if(bankManagement.deposit(depo,currAc)) {
								System.out.println("Deposit successfull");
							}
							else {
								System.out.println("ERR: Deposit was unsuccessfull");
							}
						}
						//withdrawl
						else if(ch == 4) {
							System.out.println("Enter the Amount u want to withdraw");
							widrw =  Integer.parseInt(sc.readLine());
							
							if(bankManagement.withdraw(widrw,currAc)) {
	...
	public static boolean deposit(int depo,int currAc) throws SQLException {
	...
	public static void getBalance(int acNo) // fetch balance method
	{
		try {
			...
			System.out.println("-----------------------------------------------------------");
			System.out.printf("%s12s %10s %10s\n",
							"Account No", "Name",
							"Balance");
